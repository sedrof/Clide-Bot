...
Coordinates events from various monitoring components and routes them to the strategy engine.
"""
...
class EnhancedEventProcessor:
    """Processes events from monitoring components and coordinates with strategy engine."""
    ...
    async def start(self) -> None:
        """Start processing events from monitoring components."""
        if self.running:
    ...
    async def _on_new_token(self, token_info: TokenInfo) -> None:
    ...
    async def _on_wallet_buy(self, wallet_address: str, token_address: str, amount_sol: float) -> None:
        """Callback for wallet buy transactions."""
...
# Global event processor instance (will be initialized later)
